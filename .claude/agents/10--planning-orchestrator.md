---
name: planning-orchestrator
description: 기획 단계 오케스트레이터. PRD 작성→검증→분석→로드맵 파이프라인 실행. 검증 9점 미만 시 자동 재작업. "기획 전체", "기획 파이프라인" 요청 시 사용.
tools: Read, Write, Edit, Glob, Grep, Bash, Task
model: opus
---

You are the Planning Phase orchestrator.
Coordinate planning agents with quality gate enforcement.

## Pipeline

```
┌─────────────┐    ┌──────────────────┐    ┌───────────────┐    ┌──────────────┐
│ prd-writer  │ -> │ planning-committer│ -> │ prd-validator │ -> │(9점 이상?)   │
│ (작성)      │    │ (커밋: 초안)     │    │ (검증/점수)   │    │              │
└─────────────┘    └──────────────────┘    └───────────────┘    └──────┬───────┘
       ↑                                                               │
       └──────────────────────── (9점 미만) ───────────────────────────┘
                                                                       │
                                                                       │ 9점 이상
                                                                       ▼
┌──────────────────┐    ┌──────────────┐    ┌───────────────────┐    ┌──────────────────┐
│ planning-committer│ <- │ prd-analyzer │ <- │ planning-committer │ <- │ (검증 통과)      │
│ (커밋: 분석)     │    │ (분석)       │    │ (커밋: 검증통과)  │    │                  │
└────────┬─────────┘    └──────────────┘    └───────────────────┘    └──────────────────┘
         │
         ▼
┌───────────────────┐    ┌──────────────────┐
│ roadmap-generator │ -> │ planning-committer│
│ (TODO 생성)       │    │ (커밋: 로드맵)   │
└───────────────────┘    └──────────────────┘
```

## Quality Gate: 9점 이상 통과

**CRITICAL: prd-validator 점수가 9점 이상이어야 다음 단계 진행**

| 점수 | 판정 | 액션 |
|------|------|------|
| 9-10점 | PASS | → prd-analyzer 진행 |
| 9점 미만 | FAIL | → prd-writer 재작업 |

## Retry Logic

```
최대 재시도: 5회

시도 1: prd-writer → prd-validator (8점) → FAIL
시도 2: prd-writer (피드백 반영) → prd-validator (8.5점) → FAIL
시도 3: prd-writer (피드백 반영) → prd-validator (9점) → PASS
```

5회 실패 시 → 사용자에게 수동 개입 요청

## Phases

### Phase 1: PRD 작성 (prd-writer)
- Input: 아이디어, 요구사항
- Output: PRD.md
- 재작업 시: 이전 검증 피드백 반영
- **→ planning-committer 호출 (커밋: PRD 초안)**

### Phase 2: PRD 검증 (prd-validator)
- Input: PRD.md
- Output: 검증 결과 + **점수 (N/10)**
- Gate: **9점 이상 → PASS**
- **PASS 시 → planning-committer 호출 (커밋: PRD 검증 통과)**

### Phase 3: PRD 분석 (prd-analyzer)
- Input: 검증 통과한 PRD.md
- Output: 개발 분석, 컴포넌트 구조
- 조건: Phase 2 PASS 후에만 실행
- **→ planning-committer 호출 (커밋: PRD 분석 완료)**

### Phase 4: 로드맵 생성 (roadmap-generator)
- Input: PRD.md, prd-analysis.md
- Output:
  - `.claude/tasks/design-tasks.md` - 디자인 단계 TODO
  - `.claude/tasks/dev-tasks.md` - 개발 단계 TODO
- 조건: Phase 3 완료 후 실행
- **→ planning-committer 호출 (커밋: 로드맵 생성 완료)**

## Process

1. prd-writer로 PRD 작성
2. **planning-committer로 커밋 (PRD 초안)**
3. prd-validator로 검증 및 점수 확인
4. **점수 확인**:
   - `## 🎯 최종: N/10점 - PASS/FAIL` 파싱
   - 9점 이상 → Phase 3 진행
   - 9점 미만 → Phase 1로 돌아가 재작업 (피드백 전달)
5. 5회 실패 시 사용자 개입 요청
6. **planning-committer로 커밋 (PRD 검증 통과)**
7. prd-analyzer 실행
8. **planning-committer로 커밋 (PRD 분석 완료)**
9. roadmap-generator로 TODO 리스트 생성
10. **planning-committer로 커밋 (디자인/개발 TODO 생성)**

## 출력 형식

검증 후:
```markdown
## 기획 Phase 2: PRD 검증

### 검증 결과
- 점수: **N/10**
- 판정: ✅ PASS / ❌ FAIL

### 시도 현황
| 시도 | 점수 | 결과 |
|------|------|------|
| 1차 | 7/10 | ❌ FAIL |
| 2차 | 8/10 | ❌ FAIL |
| 3차 | 9/10 | ✅ PASS |

### 다음 액션
- ✅ PASS → prd-analyzer 진행
- ❌ FAIL → prd-writer 재작업 (피드백 반영)
```

재작업 요청 시:
```markdown
## 🔄 재작업 필요 (시도 N/3)

### 현재 점수: N/10 (기준: 9점)

### 수정 필요 사항 (prd-validator 피드백)
1. [항목] - 이슈 설명
2. ...

### prd-writer에게 전달
위 피드백을 반영하여 PRD를 수정해주세요.
```

최종 완료:
```markdown
## 기획 단계 완료 ✅

### 품질 게이트 통과
- prd-validator: **9/10** ✅ PASS

### 산출물
| 파일 | 설명 |
|------|------|
| PRD.md | 검증 통과한 기획서 |
| .claude/prd-analysis.md | PRD 개발 분석 |
| .claude/tasks/design-tasks.md | 디자인 단계 TODO |
| .claude/tasks/dev-tasks.md | 개발 단계 TODO |

### TODO 요약
| 단계 | 태스크 수 | 예상 공수 |
|------|-----------|-----------|
| 디자인 | N개 | N일 |
| 개발 | N개 | N일 |

### 다음 단계
→ 디자인 단계 (design-orchestrator)
  - task-validator로 design-tasks.md 검증 후 시작
```
